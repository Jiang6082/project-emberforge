...
def candidate_report_md(report: dict) -> str:
    ...
    lines = [
        ...
        f"| capacity (USD, impact→0 alpha) | {_fmt(m.get('capacity_usd'), 0)} |",
        "",
        ...
        f"| PBO — CSCV (family) | {_fmt(s.get('pbo'), 3)} |",
        f"| PBO — CPCV (family) | {_fmt(s.get('pbo_cpcv'), 3)} |",
    ...
    lines += [
        ...
        *[f"- {r}" for r in report["decision_reasons"]],
        ...
        "- Diagnostic portfolio is not executable; costs and capacity are modeled "
        "approximately (see docs/COST_AND_CAPACITY.md), not live-execution frictions.",
