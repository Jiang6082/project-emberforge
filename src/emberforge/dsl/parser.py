"""A tiny, safe recursive-descent parser for factor expressions.
...
Function names map to operators in :mod:`emberforge.dsl.operators`. Binary
operators desugar to calls (``a + b`` -> ``Call('add', (a, b))``). This never
uses ``eval``; the only thing that can come out is a :mod:`nodes` tree.
"""
