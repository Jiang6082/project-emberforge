"""Operator registry and causal implementations.
...
* ``commutative``— used by canonicalization to sort argument order.
* ``fn``         — a pure function over pandas objects.

Data convention: a "panel" is a ``pandas.DataFrame`` indexed by timestamp
(ascending) with one column per symbol. Time-series operators act along the
index (rows = time); cross-sectional operators act within each row (axis=1);
arithmetic is elementwise. Nothing here can look forward: time-series ops use
trailing windows and non-negative shifts only.
"""
